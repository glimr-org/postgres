...
fn with_pool(f: fn(db.DbPool) -> a) -> a {
  ...
  let _ =
    db.get_connection(pool, fn(conn) {
      let _ = db.exec_with(conn, "DROP TABLE IF EXISTS unique_users", [])
      let assert Ok(_) =
        db.exec_with(
          conn,
          "CREATE TABLE unique_users (id SERIAL PRIMARY KEY, email TEXT, api_key TEXT)",
          [],
        )
      let assert Ok(_) =
        db.exec_with(
          conn,
          "INSERT INTO unique_users (email, api_key) VALUES ('admin@example.com', 'secret-KEY-123')",
          [],
        )
    })
...
pub fn unique_passes_when_value_not_in_db_test() {
  with_pool(fn(pool) {
    ...
    validator.start(
      [validator.for("email", [validator.Unique(pool, "unique_users")])],
      data,
      ctx,
    )
    |> should.be_ok
    ...
    validator.start(
      [
        validator.for("api_key", [
          validator.UniqueSensitive(pool, "unique_users"),
        ]),
      ],
      data,
      ctx,
