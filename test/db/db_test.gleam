...
fn with_pool(f: fn(db.DbPool) -> a) -> a {
  ...
  let _ =
    db.get_connection(p, fn(conn) {
      let _ = db.exec_with(conn, "DROP TABLE IF EXISTS accounts", [])
      let assert Ok(_) =
        db.exec_with(
          conn,
...
pub fn transaction_commits_on_success_test() {
  with_pool(fn(p) {
    ...
    db.get_connection(p, fn(conn) {
      ...
      let assert Ok(db.QueryResult(_, [balance])) =
...
pub fn transaction_returns_value_test() {
  with_pool(fn(p) {
    let result =
      db.transaction(p, 0, fn(conn) {
        let assert Ok(_) =
          db.exec_with(conn, "INSERT INTO accounts VALUES (1, 50)", [])
  ...
  with_pool(fn(p) {
    ...
    let result =
      db.transaction(p, 0, fn(conn) {
        ...
        db.exec_with(conn, "INSERT INTO nonexistent_table_xyz VALUES (1)", [])
      })
...
pub fn transaction_multiple_operations_test() {
  with_pool(fn(p) {
    ...
    db.get_connection(p, fn(conn) {
      ...
      let assert Ok(db.QueryResult(_, rows)) =
